...
# Import server components - using lazy imports to avoid module conflicts
run_mcp_server = None
run_http_server = None


def _get_mcp_server():
    """Lazy import of MCP server main function."""
    global run_mcp_server
...
def _get_http_server():
    """Lazy import of HTTP server function."""
    global run_http_server
...
def has_mcp_server():
    ...
    return _get_mcp_server() is not None


def has_http_server():
    """Check if HTTP server is available."""
    return _get_http_server() is not None
