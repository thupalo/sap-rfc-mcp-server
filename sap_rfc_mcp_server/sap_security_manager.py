...
from sap_rfc_mcp_server.secure_config import (
...
def main():
    ...
    test_parser.add_argument(
        "--method",
        choices=["auto", "env", "dotenv", "keyring", "encrypted_file"],
        default="auto",
        help="Configuration method to test",
    )
    test_parser.add_argument(
        "--file", type=Path, help="Path to encrypted file (for encrypted_file method)"
    )
    ...
    migrate_parser = subparsers.add_parser(
        "migrate", help="Migrate credentials between storage methods"
    )
    ...
    migrate_parser.add_argument(
        "--from-file", type=Path, help="Source encrypted file path"
    )
    migrate_parser.add_argument("--from-password", help="Source file password")
    migrate_parser.add_argument(
        "--to-file", type=Path, help="Target encrypted file path"
    )
    ...
    try:
        if args.command == "setup":
            ...
            print(f"\n[OK] Configuration completed: {config}")

        elif args.command == "test":
            print(f"[SEARCH] Testing credential configuration (method: {args.method})")

            kwargs = {}
            if args.method == "encrypted_file":
                if not args.file or not args.password:
                    print(
                        "[ERROR] --file and --password required for encrypted_file method"
                    )
                    return
            ...
            if "RFCSI_EXPORT" in system_info:
                ...
                print(f"[OK] SAP Connection successful!")
                ...
                print(
                    "[WARNING] SAP connection established but system info unavailable"
                )

        elif args.command == "info":
            print("[EMOJI] AgentSAP Security Information")
            print("=" * 40)
            ...
            print(f"[PACKAGE] Available Security Methods:")
            print(f"   Environment Variables: [OK] Always available")
            print(
                f"   .env Files: {'[OK]' if DOTENV_AVAILABLE else '[ERROR]'} (python-dotenv)"
            )
            print(
                f"   System Keyring: {'[OK]' if KEYRING_AVAILABLE else '[ERROR]'} (keyring)"
            )
            print(f"   Encrypted Files: [OK] Always available (cryptography)")
            ...
            try:
                ...
                print(f"   Source: {security_info['source']}")
                print(f"   Encrypted: {security_info['encrypted']}")
                print(f"   Status: [OK] Credentials available")
            ...
            print(
                f"[REFRESH] Migrating credentials from {args.from_method} to {args.to_method}"
            )
            ...
            if args.from_method == "encrypted_file":
                if not args.from_file or not args.from_password:
                    print("[ERROR] --from-file and --from-password required")
                    return
                from_kwargs = {
            ...
            print(f"[OK] Loaded credentials from {args.from_method}")
            ...
            if args.to_method == "keyring":
                ...
                if not args.to_file or not args.to_password:
                    print("[ERROR] --to-file and --to-password required")
                    return
                ...
                with open(env_file, "w") as f:
                    ...
                    f.write(f"SAP_PASSWD={config.passwd}\n")
